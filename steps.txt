...
Practice 1 – VPC + CloudFormation (IaC Best Practices)
   ...
   aws cloudformation create-stack --stack-name LabVPC-Demo \
     ...
     --disable-rollback 
   aws cloudformation create-stack --stack-name LabVPC-Demo --parameters ParameterKey=InstanceType,ParameterValue=t2.micro --template-body file://Enterprise_VPC_Complete_working.yaml --disable-rollback 
...
Practice 2 – AWS CodeDeploy (EC2 + Agent + Blue/Green potential)
   ...
   aws ssm list-command-invocations --command-id <paste-command-id> --details --region ap-south-1
...
Practice 3 – CodePipeline (Source → Build → Deploy)
...
Practice 4 – AWS SAM (Serverless)
   ...
   → Check pipeline trigger in CodePipeline or SAM pipeline
...
Practice 5 – ECS / Fargate Pipeline
   ...
   → Check ECR repo (image pushed), CodePipeline stages, ECS service update
